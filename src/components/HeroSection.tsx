...
export const HeroSection = () => {
    ...
    const highlightWords = Number(t('hero_highlight_words') || 3);

    const words = heroTitle.trim().split(/\s+/); // режем по любому пробельному символу
    const firstPart = words.slice(0, highlightWords).join(' ');
    const restPart = words.slice(highlightWords).join(' ');
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                    ...
                    <div className="space-y-6 animate-slide-in-left">
                        <h1 className="text-6xl lg:text-8xl font-bold leading-none">
                            <span className="block rotate-chaos-1 inline-block text-accent">
                                {firstPart}
                            </span>
                            <br/>
                            <span className="block rotate-chaos-2 inline-block mt-2">
                                {restPart}
                            </span>
                        </h1>
                        ...
                        <div className="absolute inset-0 flex items-center justify-center">
                            <div className="relative">
                                ...
                                <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
                                    <div
                                        className="absolute top-4 right-4 w-20 h-20 bg-paper rotate-12 collage-shadow paper-texture animate-float"
                                        style={{animationDelay: '1s'}}/>
                                    <div
                                        className="absolute bottom-8 left-8 w-24 h-16 bg-accent/20 -rotate-6 collage-shadow animate-float"
                                        style={{animationDelay: '1.5s'}}/>
                                    <div
                                        className="absolute top-1/2 left-0 w-16 h-16 bg-muted rotate-45 collage-shadow animate-float"
